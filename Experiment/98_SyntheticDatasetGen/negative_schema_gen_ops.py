...
def generate_negative_schema(goal_schema, operation, path, new_schema, new_key):
    ...
    if operation == "anyOf_change_type":
        return neg_anyOf_change_type(goal_schema, path)
    if operation == "anyOf_add_child":
        return neg_anyOf_add_child(goal_schema, path)
...
def neg_anyOf_change_type(goal_schema, path):
    ...
    def change_type(goal_schema, target, path):
        ...
        # elif mode == 2:
        new_schema = get_random_subschema(goal_schema, path, mode)

        return new_schema

    return neg_template(change_type, "anyOf_change_type", goal_schema, path)

def neg_anyOf_add_child(goal_schema, path):
    ...
    def add_element(goal_schema, target, path):
        ...
        if "items" in target:
            ...
            raise ValueError("No items or prefixItems in target")

        return target

    return neg_template(add_element, "anyOf_add_child", goal_schema, path)
