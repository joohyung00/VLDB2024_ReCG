...
class InstanceCluster
{
    private:
        ...
        SchemaSet*                          children_schema_set_;
        SchemaSeq*                          children_schema_seq_;
        int                                 children_num_;
            // Com
        int                                 children_num_com_kleene_;
        SchemaSet*                          children_schemas_com_kleene_;

        // Hom
        LabelToSchemaSet*                   label_to_schemas_;
    ...
    public:
        ...
        void initiateInstanceClusterVariables()
        {
            ...
            children_schema_set_            = nullptr;
            children_schema_seq_            = nullptr;
            children_num_                   = 0;
            children_num_com_kleene_        = 0;
            children_schemas_com_kleene_    = nullptr;
            label_to_schemas_               = nullptr;
        ...
        void freeMetadataResources()
        {
            ...
            if(children_schema_set_ != nullptr)
            {
                delete children_schema_set_;
                children_schema_set_ = nullptr;
            }
            if(children_schema_seq_ != nullptr)
            {
                delete children_schema_seq_;
                children_schema_seq_ = nullptr;
            }
            if(children_schemas_com_kleene_ != nullptr)
            {
                delete children_schemas_com_kleene_;
                children_schemas_com_kleene_ = nullptr;
            }
            if(label_to_schemas_ != nullptr)
            {
                delete label_to_schemas_;
                label_to_schemas_ = nullptr;
            }
        ...
        void setChildrenSchemaSet(SchemaSet& schema_set)
        { 
            children_schema_set_ = new SchemaSet();
            *children_schema_set_ = schema_set; 
        }

        SchemaSet* getChildrenSchemaSet()
        { return children_schema_set_; }

        SchemaSeq* getChildrenSchemaSeq()
        { return children_schema_seq_; }

        SchemaSet* getChildrenSchemaForCom()
        { return children_schemas_com_kleene_; }

        Count getChildrenNumForCom()
        { return children_num_com_kleene_; }

        LabelToSchemaSet* getLabelsToSchemas()
        { return label_to_schemas_; }
