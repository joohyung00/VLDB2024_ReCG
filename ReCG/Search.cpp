    ...
    while(true)
    {
        if(current_level[0]->isLeafState())
        {
            ...
            for(auto& candidate : current_level)
            {
                ...
                /* ORIGINAL */
                if(candidate->getCost() < current_optimal->getCost())
                {
                    current_optimal = candidate;
                }
            }
            cout << "NODES THAT DERIVED: #" << nodes_that_derived << endl;
...
StateNode* greedySearch(StateNode* start_state)
{
    ...
    while(!current_state->isLeafState())
    {
        ...
        {
            if(candidate->getCost() < most_greedy->getCost())
            {
...
StateNode* branchAndBoundSearch(StateNode* start_state, int beam_size)
{
    ...
    while(!state_stack.empty())
    {
        ...
        if(current_state->isLeafState())
        {
            ...
            else if(current_state->getCost() < current_optimal_state->getCost())
            {
                current_optimal_state = current_state;
            }
        }
        ...
        {
            if(current_optimal_state != nullptr && current_state->getCost() > current_optimal_state->getCost())
            {   
            ...
            else
            {
                ...
                // cut by beam size
                if(children_states.size() > beam_size)
                {
                    children_states.resize(beam_size);
                }
