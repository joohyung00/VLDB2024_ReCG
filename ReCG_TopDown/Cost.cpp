...
MDLCost homObjSRC(InstanceCluster& hom_obj_cluster, CostParameters& cost_parameters)
{
    ...
    // 2. Number of symbols
    int labels_num = hom_obj_cluster.getLabelToTypes()->size();
...
MDLCost comObjSRC(InstanceCluster& com_obj_cluster, CostParameters& cost_parameters)
{
    ...
    // 2. Number of symbols
    int labels_num = com_obj_cluster.getLabelToTypes()->size();
...
MDLCost homArrSRC(InstanceCluster& hom_arr_cluster, CostParameters& cost_parameters)
{
    ...
    // 2. Number of symbols
    int sch_children_num = hom_arr_cluster.getChildrenTypeSeq()->size();
...
MDLCost homObjKSE(InstanceCluster& hom_obj_cluster, CostParameters& cost_parameters)
{
    ...
    for(auto& instance : *hom_obj_cluster.getInstanceForest())
    {
    ...
    // 3. Calculate entropy
    MDLCost entropy = 0;
    for(auto& count : label_to_count)
    {
        if(count != 0)
        {
            double p = (double)count / hom_obj_cluster.getForestSize();
            entropy += -p * log2(p);
        }
    }
...
MDLCost hetObjKSE(InstanceCluster& het_obj_cluster, CostParameters& cost_parameters)
{
    return homObjKSE(het_obj_cluster, cost_parameters);
}

MDLCost comObjKSE(InstanceCluster& com_obj_cluster, CostParameters& cost_parameters)
{
    return homObjKSE(com_obj_cluster, cost_parameters);
}

MDLCost homArrKSE(InstanceCluster& hom_arr_cluster, CostParameters& cost_parameters)
{
    ...
    for(auto& instance : *hom_arr_cluster.getInstanceForest())
    ...
    for(auto& count : arr_len_to_count)
    {
        if(count != 0)
        {
            double p = (double)count / hom_arr_cluster.getForestSize();
            entropy += -p * log2(p);
        }
    }
...
MDLCost hetArrKSE(InstanceCluster& het_arr_cluster, CostParameters& cost_parameters)
{
    return homArrKSE(het_arr_cluster, cost_parameters);
}
