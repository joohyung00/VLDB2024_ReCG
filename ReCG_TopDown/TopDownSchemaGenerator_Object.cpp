...
void TopDownSchemaGenerator::clusterAndGeneralizeObjects()
{
    ...
    // 1. First filter out empty objects
...
void TopDownSchemaGenerator::preprocessInstances(vector<bool>& mask, int& instance_num)
{
    InstanceForest& object_forest = grouped_instances_->getInstanceForestByType(kObject);
...
// void TopDownSchemaGenerator::filterHeterogeneousObjects(vector<bool>& mask, int& instance_num)
// {
...
void TopDownSchemaGenerator::clusterCDObjects(vector<bool>& mask, int& instance_num)
{
    InstanceForest& object_forest = grouped_instances_->getInstanceForestByType(kObject);
...
void TopDownSchemaGenerator::generalizeOutlierObjects(vector<bool>& mask, int& unmasked_num)
{
    InstanceForest& object_forest = grouped_instances_->getInstanceForestByType(kObject);
...
void TopDownSchemaGenerator::clusterGeneralizedOutlierObjects(vector<bool>& mask, int& instance_num)
{
    #if VERBOSE2
      printHeader();
...
void TopDownSchemaGenerator::treatEdgeCases(vector<bool>& mask, int& instance_num)
{
    InstanceForest& object_forest = grouped_instances_->getInstanceForestByType(kObject);
...
void TopDownSchemaGenerator::linearlyFilterObjects(
...
)
{
    ...
    for(auto& cluster_object : biggest_cluster)
    {
        ...
        roaring_bitmap_t* key_schema = cluster_object->getLabelChildtypeBitmap();
        roaring_bitmap_t* kleene_schema = cluster_object->getKleenedChildtypes();
    ...
    for(int i = 0; i < object_forest.size(); i++)
    {
        ...
        roaring_bitmap_t* key_schema = object_forest[i]->getLabelChildtypeBitmap();
        roaring_bitmap_t* kleene_schema = object_forest[i]->getKleenedChildtypes();
...
void TopDownSchemaGenerator::updateClustersMetadata()
{
...
// void TopDownSchemaGenerator::filterHeterogeneousClusters()
// {
...
//                 SchemaSet* children_schemas = object_clusters_[i].getChildrenTypeSet();
//                 bool all_primitives = true;
...
//             // for(auto& element : *object_clusters_[i].getChildrenTypeSet())
//             // {
//             //     cout << element << " ";
//             // }
//             // cout << endl;
//             // for(auto& element : *object_clusters_[target_idx].getChildrenTypeSet())
//             // {
//             //     cout << element << " ";
//             // }
//             // cout << endl;

//             if(isEqual(object_clusters_[i].getChildrenTypeSet(), object_clusters_[target_idx].getChildrenTypeSet()))
//             { 
//                 // cout << "EQUALED" << endl;
