...
void TopDownSchemaGenerator::generalizeAndClusterArrays()
{
...
void TopDownSchemaGenerator::clusterCDArrays(vector<bool>& mask, int& instance_num)
{
    InstanceForest& array_forest = grouped_instances_->getInstanceForestByType(kArray);
...
void TopDownSchemaGenerator::linearlyFilterArrays(
...
)
{
    ...
    for(int i = 0; i < array_forest.size(); i++)
    {
        ...
        roaring_bitmap_t* kleene_schema = array_forest[i]->getKleenedChildtypes();
...
void TopDownSchemaGenerator::findHomArrayCluster()
{
