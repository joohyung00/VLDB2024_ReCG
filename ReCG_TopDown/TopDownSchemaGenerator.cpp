...
StateMaterials TopDownSchemaGenerator::deriveNextSchema()
{
    ...
    else if(derivation_phase_ == kMergingPhase)
    {
        ...
        if(!merged)
        {
            ...
            // 2.3. Free resources for clusters
                // This state(top_down_schema_generator_) does not need resources anymore
        ...
        IllegalBehaviorError("TopDownSchemaGenerator::deriveNextSchema - Illegal State");

    // 3. Derive schemas from each cluster
        // 3.1. Derive schema(recorded in each instances)
    deriveSchemasFromClusters();
    new_state_materials.setStateId(TopDownSchemaGenerator::unique_state_id_);
    TopDownSchemaGenerator::unique_state_id_++;
...
void TopDownSchemaGenerator::clusterAllGenericTypes()
{
    #if VERBOSE
      printHeader();
...
// Object Clustering in `TopDownSchemaGenerator.cpp`
// Array  Clustering in `TopDownSchemaGenerator_Array.cpp`
// Merging           in `TopDownSchemaGenerator.cpp`

void TopDownSchemaGenerator::deriveSchemasFromClusters()
{
...
void TopDownSchemaGenerator::printHeader()
{
